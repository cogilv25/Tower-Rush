...
{
	// These doors essentially form a linked list, the mechanism
	//    is an invisible surface for the prismatic joint to attach to,
	//    both the stoppers are used to trigger the next door on
	//    contact events.
	if(reverse)
...
function initializeComplexEntities()
{
	...
	// TODO: Breakable Doors

	// Heavy Doors
	  // Single Heavy
	let door2 = createHeavyDoor(1, 1, false, 3, 300, 4320, 235, 120);
	  // Double Heavy
	let door4 = createHeavyDoor(2, 1, false, 1, 2000, 4860, 1704, 264);
	  // Triple Heavy
	let door3 = createHeavyDoor(3, 1, true, 5, 1500, 4264, 69, 120);
	  // Quad Heavy
	let door1 = createHeavyDoor(4, 1, false, 6, 2000, 600, 152, 288);
	...
	createEntity(entityTypes['pressure_plate'], { x: 5032, y: 1567, target: elevator, weightLimit: 3, weight: 0});
	...
	createEntity(entityTypes['pressure_plate'], { x: 4598, y: 1567, target: door4, weightLimit: 3, weight: 0});

	// Ladders
	createLadder(3920, 18, 6);

	// Player Counters
	createEntity(entityTypes['player_counter'], {x: 4480, y: 1692, width: 512, height: 264});
	complexState[entities.length - 1].activationFunction = null;//gameComplete;
	complexState[entities.length - 1].activationEnergy = 2;
	complexState[entities.length - 1].count = 0;

	// TODO: Replace below with a more reliable solution based on a player_counter.
...
function attachColliders(playerId)
{
	...
	// Same idea for the 2 dash colliders (forwards and backwards)
	//    except these are translated a distance away from the body
	//    to check for collisions where the player will dash to.
	width = entityTypes["player_s"].width;
	height = entityTypes["player_s"].height;

	fixture.userData = {type: "player_dash_collider", id: playerId, dir: -1};
    fixture.shape = new Polygon;
    fixture.shape.SetAsBox((width / scale) / 1.75, (height / scale) / 2);

    for(let i = 0; i < fixture.shape.m_vertices.length; ++i)
    {
    	fixture.shape.m_vertices[i].x -= (width * 8) / scale;
    }
	...
	for(let i = 0; i < fixture.shape.m_vertices.length; ++i)
    {
    	fixture.shape.m_vertices[i].x += (width * 16) / scale;
    }
    fixture.userData = {type: "player_dash_collider", id: playerId, dir: 1};
...
function initialize()
{
	...
	// TODO: This should be placing player spawns rather than players...
...
function balloonSpecial(playerId, activate)
{
	if(activate)
	{
		...
		if(playerInputs[INPUT.RIGHT_ACTION] || playerInputs[INPUT.RIGHT_ACTION] || playerInputs[INPUT.JUMP_ACTION])
			return;
...
function fazeDash(playerId)
{
	...
	// If we would collide with something at the end of the dash
	//    don't allow it.
	if(players[playerId].dashColliderCount[(players[playerId].direction + 1) % 2] != 0)
		return;

	players[playerId].specialSauce -= 60;
	let b = entities[playerId].GetBody();
	let vel = b.GetLinearVelocity();
	entities[playerId].m_filter.maskBits = 16;
	b.m_force.y = -gravity.y * b.GetMass();
	b.m_linearVelocity.x += entityTypes["player_s"].width * players[playerId].direction;
	b.m_linearVelocity.y = 0;
	players[playerId].activeSpecial = "faze_dash";
	players[playerId].specialTimer = setTimeout(() => 
		{
			players[playerId].specialTimer = null;
			players[playerId].activeSpecial = "none";
			...
			entities[playerId].m_filter.maskBits = 65533;
		}
	, 200);
}
...
function update()
{
	...
	if(players[0].dieOnCollisionFor1Frame) players[0].dieOnCollisionFor1Frame = false;
	if(players[1].dieOnCollisionFor1Frame) players[1].dieOnCollisionFor1Frame = false;
...
listener.BeginContact = function(contact) 
{
	...
	if((u1.type == "player" || u2.type == "player") && (u1.type == "player_counter" || u2.type == "player_counter"))
	{
		let counter = (u1.type == "player_counter" ? u1 : u2).index;
		if(++complexState[counter].count >= complexState[counter].activationEnergy)
		{
			if(complexState[counter].activationFunction != null)
			{
				if(complexState[counter].activated != true)
				{
					complexState[counter].activationFunction();
					complexState[counter].activated = true;
				}
			}
		}
	}
	...
	if(
		...
		(u1.type != "player_dash_collider" && u2.type != "player_dash_collider") &&
		(u1.type != "player_counter" && u2.type != "player_counter")
	)
	{
		let pId = (u1.type == "player_balloon_collider" ? u1 : u2).id;
		++players[pId].balloonColliderCount;
	}

	if(
		((u1.type == "player_dash_collider" && u2.type != "player_dash_collider") || 
		(u2.type == "player_dash_collider" && u1.type != u1.type == "player_dash_collider")) &&
		((u1.type != "player_balloon_collider" && u2.type != "player_balloon_collider") &&
		(u1.type != "player_counter" && u2.type != "player_counter"))
	)
	{
		let ud = (u1.type == "player_dash_collider" ? u1 : u2);
		let ty = (u1.type == "player_dash_collider" ? u2 : u1).type;
		let pId = ud.id;
		let dir = ud.dir;
	}
